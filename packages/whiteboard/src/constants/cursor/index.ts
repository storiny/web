 ...
 */
const wrap_svg = (data: string, style?: string): string =>
 ...
 * @param svg_string Cursor SVG markup
 * @param fallback Native cursor fallback
 * @param offset Cursor offset
 */
const get_cursor_style = (
  svg_string: string,
  fallback: CSSProperties["cursor"] = "default",
  offset = "16 16"
): string =>
  `url("data:image/svg+xml;base64,${btoa(svg_string)}") ${offset}, ${fallback}`;

/**
 * Returns reusable resize cursor
 * @param angle Cursor angle
 */
const get_resize_cursor = (angle?: number): string =>
  wrap_svg(
 ...
 * @param angle Cursor angle
 */
const get_rotate_cursor = (angle?: number): string =>
  wrap_svg(
 ...
 * @param fill Pen fill
 */
const get_pen_cursor = (fill = "#050505"): string =>
  wrap_svg(
...
export const CURSORS = {
  ...
  rotate: (angle?: number) =>
    get_cursor_style(get_rotate_cursor(angle), "crosshair"),
  "n-resize": get_cursor_style(get_resize_cursor(), "n-resize"),
  "s-resize": get_cursor_style(get_resize_cursor(), "s-resize"),
  "nw-resize": get_cursor_style(get_resize_cursor(-45), "ne-resize"),
  "se-resize": get_cursor_style(get_resize_cursor(-45), "sw-resize"),
  "ne-resize": get_cursor_style(get_resize_cursor(45), "ne-resize"),
  "sw-resize": get_cursor_style(get_resize_cursor(45), "sw-resize"),
  "w-resize": get_cursor_style(get_resize_cursor(90), "w-resize"),
  "e-resize": get_cursor_style(get_resize_cursor(90), "e-resize")
} as const;
