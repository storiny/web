import {
  BaseBrush,
  Canvas,
  classRegistry as class_registry,
  Point,
  Shadow,
  TEvent,
  util
...
import { getStroke as get_stroke, StrokeOptions } from "perfect-freehand";
 ...
 */
const convert_points_to_svg_commands = (
 ...
 */
const get_svg_path_from_points = (
  points: Point[],
  options?: StrokeOptions & { strokeWidth: number }
): TSimplePathData =>
  convert_points_to_svg_commands(
    get_stroke(points, {
      thinning: 0.6,
      smoothing: 0.5,
      streamline: 0.5,
...
export class PenBrush extends BaseBrush {
   ...
   * @private
   */
  private has_committed_last_point: boolean;
  /**
   * Pen style
   */
  ...
  onMouseMove(pointer: Point, { e }: TEvent): void {
    ...
    if (this.add_point(pointer) && this.points.length > 1) {
  ...
  onMouseUp({ e }: TEvent): boolean {
    ...
    this.has_committed_last_point = true;
    this.finalize_and_add_path();
   ...
   * @private
   */
  private prepare_for_drawing(pointer: Point): void {
    this.reset();
    this.add_point(pointer);
   ...
   * @private
   */
  private set_brush_styles(ctx: CanvasRenderingContext2D): void {
   ...
   * @private
   */
  private add_point(point: Point): boolean {
    if (
  ...
  private reset(): void {
    this.points = [];
    this.set_brush_styles(this.canvas.contextTop);
    this._setShadow();
  }
   ...
   * @private
   */
  private get_svg_path_data(): string {
    return util.joinPath(
      get_svg_path_from_points(this.points, {
        last: this.has_committed_last_point,
   ...
   * @private
   */
  private finalize_and_add_path(): void {
    const path = this.create_path();
  ...
  _render(ctx: CanvasRenderingContext2D = this.canvas.contextTop): void {
    ...
    ctx.fill(new Path2D(this.get_svg_path_data()));
    ctx.restore();
  }
}
...
export class Pen extends PenPrimitive<PenProps> {
  ...
  constructor(props: PenProps) {
    super(
      get_svg_path_from_points(props.points || [], {
        last: true,
  ...
  _render(ctx: CanvasRenderingContext2D): void {
    const path_data = get_svg_path_from_points(this.get("points"), {
      last: true,
      strokeWidth: this.get("penWidth") || 1,
      simulatePressure: this.get("penStyle") === PenStyle.PRESSURE
    });

    this._setPath(path_data);
    super._render.apply(this, [ctx]);
  }
}

class_registry.setClass(Pen, LayerType.PEN);
