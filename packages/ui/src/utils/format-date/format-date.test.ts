...
import { DateFormat, dayjs, format_date } from "./format-date";

type Fixture = {
  expected: string;
  format: DateFormat | null;
  value: Parameters<typeof format_date>[0];
};
...
const RELATIVE_FIXTURES: Array<RelativeFixture> = [
  {
...
describe("format_date", () => {
  it.each(FIXTURES)(
    ...
    ({ value, format, expected }) =>
      expect(format_date(value, format || undefined)).toEqual(expected)
  );

  it.each([
    ...RELATIVE_FIXTURES.map((fixture) => ({
      ...fixture,
      format: DateFormat.RELATIVE
    })),
    ...RELATIVE_FIXTURES.map(({ expected, ...rest }) => ({
      ...rest,
    ...
    ({ value, unit, format, expected }) =>
      expect(
        format_date(dayjs().subtract(value, unit), format || undefined)
      ).toEqual(expected)
  );
});
