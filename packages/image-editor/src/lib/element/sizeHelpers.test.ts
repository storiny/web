...
describe("getPerfectLayerSize", () => {
  it("should return height:0 if `layerType` is line and locked angle is 0", () => {
    const { height, width } = getPerfectLayerSize("line", 149, 10);
    expect(width).toBeCloseTo(149, EPSILON_DIGITS);
    expect(height).toBeCloseTo(0, EPSILON_DIGITS);
  });
  it("should return width:0 if `layerType` is line and locked angle is 90 deg (Math.PI/2)", () => {
    const { height, width } = getPerfectLayerSize("line", 10, 140);
    expect(width).toBeCloseTo(0, EPSILON_DIGITS);
    expect(height).toBeCloseTo(140, EPSILON_DIGITS);
  });
  it("should return height:0 if `layerType` is arrow and locked angle is 0", () => {
    const { height, width } = getPerfectLayerSize("arrow", 200, 20);
    expect(width).toBeCloseTo(200, EPSILON_DIGITS);
    expect(height).toBeCloseTo(0, EPSILON_DIGITS);
  });
  it("should return width:0 if `layerType` is arrow and locked angle is 90 deg (Math.PI/2)", () => {
    const { height, width } = getPerfectLayerSize("arrow", 10, 100);
    expect(width).toBeCloseTo(0, EPSILON_DIGITS);
    expect(height).toBeCloseTo(100, EPSILON_DIGITS);
  });
  it("should return adjust height to be width * tan(locked angle)", () => {
    const { height, width } = getPerfectLayerSize("arrow", 120, 185);
  ...
  it("should return height equals to width if locked angle is 45 deg", () => {
    const { height, width } = getPerfectLayerSize("arrow", 135, 145);
  ...
  it("should return height:0 and width:0 when width and height are 0", () => {
    const { height, width } = getPerfectLayerSize("arrow", 0, 0);
    expect(width).toBeCloseTo(0, EPSILON_DIGITS);
    expect(height).toBeCloseTo(0, EPSILON_DIGITS);
  });
