...
import { tryParseCells, tryParseNumber, VALID_SPREADSHEET } from "./chart";
...
describe("chart", () => {
  describe("tryParseNumber", () => {
    it.each<[string, number]>([
      ...
      ["-$1", -1],
      ["$-1", -1]
    ])("correctly identifies %s as number", (given, expected) => {
      expect(tryParseNumber(given)).toEqual(expected);
    });

    it.each<[string]>([["a"], ["$"], ["$a"], ["-$a"]])(
      "correctly identifies %s as not a number",
      (given) => {
  ...
  describe("tryParseCells", () => {
    it("parses a spreadsheet", () => {
      ...
      const { title, labels, values } = (
        result as { spreadsheet: Spreadsheet; type: typeof VALID_SPREADSHEET }
      ).spreadsheet;
    ...
    it("treats the first column as labels if both the columns are numbers", () => {
      const spreadsheet = [
        ["time", "value"],
        ["01", "61"],
        ["02", "-60"],
