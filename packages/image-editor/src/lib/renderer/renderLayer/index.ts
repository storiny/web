...
/**
 * Predicate function for determining pending image layers
 * @param layer
 * @param renderConfig
 */
...
/**
 * Returns capped layer canvas size
 * @param layer Layer
...
const cappedLayerCanvasSize = (
  ...
  // based on the browser / device type.
  ...
  // Rescale to ensure width and height is within the limits
  if (
  ...
  // Rescale to ensure canvas area is within the limits
...
IMAGE_PLACEHOLDER_IMG.src = `data:${ImageMime.SVG},${encodeURIComponent(
  `<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="image" class="svg-inline--fa fa-image fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="#888" d="M464 448H48c-26.51 0-48-21.49-48-48V112c0-26.51 21.49-48 48-48h416c26.51 0 48 21.49 48 48v288c0 26.51-21.49 48-48 48zM112 120c-30.928 0-56 25.072-56 56s25.072 56 56 56 56-25.072 56-56-25.072-56-56-56zM64 384h384V272l-87.515-87.515c-4.686-4.686-12.284-4.686-16.971 0L208 320l-55.515-55.515c-4.686-4.686-12.284-4.686-16.971 0L64 336v48z"></path></svg>`
)}`;

const IMAGE_ERROR_PLACEHOLDER_IMG = document.createElement("img");
IMAGE_ERROR_PLACEHOLDER_IMG.src = `data:${ImageMime.SVG},${encodeURIComponent(
  ...
  switch (layer.type) {
    ...
    case LayerType.IMAGE: {
      ...
      if (img != null && !(img instanceof Promise)) {
        ...
        drawImagePlaceholder(layer, context);
      }
...
type LayerShapes = {
  [LayerType.ARROW]: Drawable[];
  [LayerType.FREE_DRAW]: Drawable | null;
  [LayerType.IMAGE]: null;
  [LayerType.LINE]: Drawable[];
  [LayerType.TEXT]: null;
};

/**
 * Returns shape for layer
 * @param layer Layer
 */
export const getShapeForLayer = <T extends Layer>(
  layer: T
): T["type"] extends keyof LayerShapes
...
): WeakMap<Layer, LayerShape> => shapeCache.set(layer, shape);

/**
 * Invalidates shape for a layer
 * @param layer Layer
 */
export const invalidateShapeForLayer = (layer: Layer): boolean =>
  shapeCache.delete(layer);

/**
 * Generates options for roughjs
 * @param layer
 * @param continuousPath
 */
export const generateRoughOptions = (
  ...
  const options: Options = {
    seed: layer.seed,
    strokeLineDash:
      layer.strokeStyle === StrokeStyle.DASHED
        ? getDashArrayDashed(layer.strokeWidth)
        : layer.strokeStyle === StrokeStyle.DOTTED
        ? getDashArrayDotted(layer.strokeWidth)
        : undefined,
    // For non-solid strokes, disable `multiStroke` because it tends to make
    // dashes/dots overlay over each other
    disableMultiStroke: layer.strokeStyle !== StrokeStyle.SOLID,
    // For non-solid strokes, increase the width a bit to make it visually
    // similar to solid strokes, because we're also disabling `multiStroke`
    strokeWidth:
      layer.strokeStyle !== StrokeStyle.SOLID
        ? layer.strokeWidth + 0.5
        : layer.strokeWidth,
    // When increasing `strokeWidth`, we must explicitly set `fillWeight` and
    // `hachureGap` because if they are not specified, roughjs uses `strokeWidth` to
...
const generateLayerShape = (
  ...
  // `null` indicates there is no rc shape applicable for this layer type
  if (shape === undefined) {
    ...
    switch (layer.type) {
      ...
      case LayerType.ARROW: {
        ...
        // Curve is always the first layer, this
        // simplifies finding the curve for a layer
        if (!layer.roundness) {
          if (options.fill) {
        ...
        if (layer.type === LayerType.ARROW) {
          const { startArrowhead = null, endArrowhead = Arrowhead.ARROW } =
            layer;
          ...
          const getArrowheadShapes = (
            ...
            if (arrowhead === Arrowhead.TRIANGLE) {
              const [x, y, x2, y2, x3, y3] = arrowheadPoints as number[];
      ...
      case LayerType.FREE_DRAW: {
        ...
        if (isPathALoop(layer.points)) {
          // Generate rough polygon to fill the freedraw shape
...
const generateLayerWithCanvas = (
  ...
  const shouldRegenerateDueToZoom =
    prevLayerWithCanvas &&
...
/**
 * Draws layer from canvas
 * @param layerWithCanvas Layer with canvas
 * @param rc Rough canvas
 * @param context Context
 * @param renderConfig Render config
 */
const drawLayerFromCanvas = (
  ...
  if (isArrowLayer(layer) && boundTextLayer) {
    ...
    // Take max dimensions of arrow canvas so that when canvas is
    // rotated, the arrow doesn't get clipped
    ...
    // We translate context to the layer center so that rotation and scale
    ...
    // Revert afterward, so that we don't have to account for it during drawing
    context.translate(-cx, -cy);
    context.drawImage(
      layerWithCanvas.canvas!,
    ...
    if (
      process.env.NEXT_PUBLIC_DEBUG_ENABLE_TEXT_CONTAINER_BOUNDING_BOX ===
        "true" &&
      hasBoundTextLayer(layer)
    ) {
      const textLayer = getBoundTextLayer(layer) as TextLayerWithContainer;
  ...
  switch (layer.type) {
    case LayerType.SELECTION: {
      ...
      if (!renderConfig.isExporting) {
        ...
        /**
         * Render from 0.5px offset to get a 1px wide line
    ...
    case LayerType.TEXT: {
      ...
      if (renderConfig.isExporting) {
        ...
        if (isArrowLayer(layer) && boundTextLayer) {
          ...
          // Take the maximum dimensions of the arrow canvas so
          // that when canvas is rotated the arrow doesn't get clipped
          const maxDim = Math.max(distance(x1, x2), distance(y1, y2));
          const padding = getCanvasPadding(layer);
          ...
          // Shift the canvas to the center of bound text
          const [, , , , boundTextCx, boundTextCy] =
            getLayerAbsoluteCoords(boundTextLayer);
        ...
        if (
          ...
          !renderConfig?.shouldCacheIgnoreZoom &&
          // When angle is 0, always disable smoothing
          (!layer.angle ||
            // If the angle is a right angle, we can still
    ...
    default: {
      throw new Error(`Unimplemented type ${(layer as Layer).type}`);
    }
  }
};

// /**
//  * Draws rough SVG with the specified precision
//  * @param rsvg Rough SVG
...
// };
// /**
//  * Renders layer to SVG
//  * @param layer Layer
//  * @param rsvg Rough SVG
//  * @param svgRoot Svg root
//  * @param files Binary files
//  * @param offsetX Offset X
//  * @param offsetY Offset Y
//  * @param exportWithDarkMode Whether to export with dark mode
//  */
...
//
//         // In dark theme, revert the image color filter
//         if (exportWithDarkMode && fileData.mimeType !== ImageMime.SVG) {
...
// };
//
export const pathsCache = new WeakMap<FreeDrawLayer, Path2D>([]);

/**
 * Generates free draw shape
 * @param layer Layer
 */
export const generateFreeDrawShape = (layer: FreeDrawLayer): Path2D => {
...
/**
 * Returns free draw path
 * @param layer Layer
 */
export const getFreeDrawPath2D = (layer: FreeDrawLayer): Path2D | undefined =>
  pathsCache.get(layer);

/**
 * Returns free draw SVG path
 * @param layer Layer
 */
export const getFreeDrawSvgPath = (layer: FreeDrawLayer): string => {
  // If input points are empty, return a dot
...
// Trim SVG path data so that numbers are two decimal points
const TO_FIXED_PRECISION = /(\s?[A-Z]?,?-?[0-9]*\.[0-9]{0,2})(([0-9]|e|-)*)/g;

/**
 * Returns SVG path from stroke
 * @param points Points
 */
