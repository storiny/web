...
describe("getPerfectLayerSize", () => {
  test("returns height:0 if `layerType` is line and locked angle is 0", () => {
  ...
  test("returns width:0 if `layerType` is line and locked angle is 90 deg (Math.PI/2)", () => {
  ...
  test("returns equal width and height if the locked angle is 45 deg", () => {
  ...
  test("returns height:0 and width:0 when width and height are 0", () => {
