...
impl<Store: SessionStore> SessionMiddlewareBuilder<Store> {
    ...
    /// Determines what type of session cookie should be used and how its lifecycle should be
    /// managed. Check out [`SessionLifecycle`]'s documentation for more details on the
    /// available options.
    ///
