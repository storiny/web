	...
	inserted_story   AS (
		INSERT
			INTO
				...
				VALUES
					('sample-story', (SELECT id FROM inserted_user), NOW())
				RETURNING id),
	inserted_comment AS (
		INSERT
			INTO
				...
				VALUES
					('Sample content', (SELECT id FROM inserted_user), (SELECT id FROM inserted_story))
				RETURNING id)
INSERT
INTO
	replies (id, content, user_id, comment_id)
VALUES
	(3, 'Sample content', (SELECT id FROM inserted_user), (SELECT id FROM inserted_comment));