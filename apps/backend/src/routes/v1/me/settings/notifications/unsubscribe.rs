...
async fn patch(
    ...
    match user.id() {
        Ok(user_id) => match NotificationEntityType::try_from(payload.r#type) {
            Ok(notification_type) => {
                ...
                query_builder.push(match notification_type {
                    NotificationEntityType::FriendReqAccept
                    | NotificationEntityType::FriendReqReceived => "push_friend_requests",
            ...
            Err(_) => Ok(HttpResponse::BadRequest().body("Invalid notification type")),
        },
