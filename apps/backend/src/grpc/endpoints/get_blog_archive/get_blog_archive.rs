...
pub async fn get_blog_archive(
    ...
    let result = sqlx::query_as::<_, Archive>(
        r#"
WITH blog AS (
...
mod tests {
    ...
    async fn can_return_blog_archive(pool: PgPool) {
        test_grpc_service(
            pool,
            false,
    ...
    #[sqlx::test(fixtures("get_blog_archive"))]
    async fn should_not_include_soft_deleted_stories_in_archive(pool: PgPool) {
        test_grpc_service(
            pool,
            false,
