	...
	FOR st_row IN
		...
		LOOP
			WITH story_tag_relation    AS (SELECT st.story_id,
				 ...
				 published_story       AS (SELECT s.user_id, u.is_private
										   FROM
											   stories s
												   INNER JOIN users u
															  ON s.user_id = u.id
										   WHERE
											   s.id = (SELECT story_id FROM story_tag_relation)
										  ),
			...
			FROM
				(SELECT tf.user_id AS "target_id"
				 FROM
					 tag_followers tf
				 WHERE
					   tf.tag_id = (SELECT tag_id FROM story_tag_relation)
				   ...
				   AND (NOT (SELECT is_private FROM published_story) IS TRUE
					 OR EXISTS(SELECT 1
							   FROM
								   friends
							   WHERE
									 (
										 (transmitter_id = tf.user_id AND
										  receiver_id = (SELECT user_id FROM published_story))
											 OR
										 (transmitter_id = (SELECT user_id FROM published_story) AND
										  receiver_id = tf.user_id)
										 )
