...
BEGIN
    ...
    IF(EXISTS(
        ...
        FROM
            users
        WHERE
            id = NEW.notified_id AND(deleted_at IS NOT NULL OR deactivated_at IS NOT NULL))) THEN
    ...
    END IF;
        ...
        IF(EXISTS(
            ...
            FROM
                mutes
                INNER JOIN notifications n ON n.id = NEW.notification_id
            WHERE
                muter_id = NEW.notified_id AND muted_id = n.notifier_id) OR EXISTS(
            SELECT
                1
            FROM
                blocks
                INNER JOIN notifications n ON n.id = NEW.notification_id
            WHERE
                blocker_id = NEW.notified_id AND blocked_id = n.notifier_id)) THEN
        ...
        END IF;
            ...
            IF(EXISTS(
                ...
                FROM
                    ...
                    INNER JOIN notifications n ON n.id = NEW.notification_id
                WHERE
                    ns.user_id = NEW.notified_id AND(
...
CREATE OR REPLACE TRIGGER notification_out_trigger
    BEFORE INSERT ON notification_outs
    FOR EACH ROW
