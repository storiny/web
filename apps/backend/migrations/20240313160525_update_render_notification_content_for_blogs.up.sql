			...
			CASE
				...
				WHEN
					...
					THEN (WITH transmitter AS (SELECT notifier.name,
						  ...
						  SELECT (
									 FORMAT('<a data-user data-fw-bold href="/%s">%s</a> accepted your friend request',
				...
				WHEN
					...
					THEN (WITH transmitter AS (SELECT notifier.name,
						  ...
						  SELECT (
									 FORMAT(
											 '<a data-user data-fw-bold href="/%s">%s</a> sent you a friend request.'
												 ||
				...
				WHEN
					...
					THEN (WITH follower AS (SELECT notifier.name,
						  ...
						  SELECT (
									 FORMAT('<a data-user data-fw-bold href="/%s">%s</a> started following you',
				...
				WHEN
					...
					THEN (WITH comment AS (SELECT c.content,
						  ...
						  SELECT (
									 FORMAT(
											 '<a data-user data-fw-bold href="/%s">%s</a>'
												 || ' commented: '
				...
				WHEN
					...
					THEN (WITH contributor AS (SELECT "sc->user".name,
						  ...
						  SELECT (
									 FORMAT(
											 '<a data-user data-fw-bold href="/%s">%s</a> accepted your collaboration request',
					...
					THEN (WITH blog AS (SELECT b.name, b.slug, b.domain
										FROM
											notification_outs n_out
												...
												INNER JOIN notifications n
														   ON n_out.notification_id = n.id
												-- Join blog editor relation
												INNER JOIN blog_editors AS be
														   ON be.id = n.entity_id
												-- Join blog
					...
					THEN (WITH blog AS (SELECT b.name, b.slug, b.domain
										FROM
											notification_outs n_out
												...
												INNER JOIN notifications n
														   ON n_out.notification_id = n.id
												-- Join blog writer relation
												INNER JOIN blog_writers AS bw
														   ON bw.id = n.entity_id
												-- Join blog
												INNER JOIN blogs AS b
														   ON b.id = bw.blog_id
										WHERE
