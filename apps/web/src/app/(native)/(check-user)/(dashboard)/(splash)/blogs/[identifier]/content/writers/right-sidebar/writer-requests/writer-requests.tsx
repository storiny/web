...
import {
  ...
  select_blog_writer_requests,
  use_get_blog_writer_requests_query
} from "~/redux/features";
import { BREAKPOINTS } from "~/theme/breakpoints";
import css from "~/theme/main.module.scss";

import styles from "./writer-requests.module.scss";
...
const WriterRequestsModal = (): React.ReactElement => {
  ...
  const refetch = use_default_fetch(
    trigger,
    { blog_id: blog.id, page, query: debounced_query },
    [blog.id, debounced_query]
  );
  const is_typing = query !== debounced_query;

  const load_more = React.useCallback(() => {
    trigger({ blog_id: blog.id, page: page + 1, query: debounced_query }, true);
  }, [blog.id, debounced_query, page, trigger]);
...
const WriterRequests = (): React.ReactElement => {
